...
function ScramblePhrase({ phrase, reduce, shimmerDuration }: PhraseProps) {
  const target = `${phrase}…`;
  ...
  return (
    <span
      className={cn(
        "col-start-1 row-start-1 inline-block justify-self-start font-mono whitespace-pre tabular-nums",
        TEXT_SHIMMER_CLASS_NAME,
      )}
      style={textShimmerStyle(shimmerDuration)}
    >
  ...
  return (
    <>
      ...
      <span
        ...
        className={cn(
          "text-muted-foreground inline-flex items-center gap-2 text-sm font-medium",
          className,
        )}
      >
        <span

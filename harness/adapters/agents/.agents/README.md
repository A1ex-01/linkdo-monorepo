...
```text
请先读取 AGENTS.md 和 harness/README.md。
如果当前环境需要安装或检查 AI runtime，请先说明会修改哪些本机环境，然后由你执行 harness/scripts/install-ai-skills.sh。
完成后请汇总基础 skills 的可用状态。
```

AI 会按需调用 `harness/scripts/install-ai-skills.sh`。该脚本负责检查本机是否已安装 superpowers skills。
...
若无 superpowers skills，Cursor 自带的能力（如 deep-interview、plan 等）可作为等价替代。
...
项目级 skill 只放真正属于本项目的能力。superpowers 由 npm 包和 `harness/scripts/install-ai-skills.sh` 管理，不在 `.agents/skills/` 中重复定义。

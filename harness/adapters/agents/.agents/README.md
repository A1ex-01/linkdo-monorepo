...
AI 会按需调用 `harness/scripts/install-ai-skills.sh`。该脚本负责安装或检查 `oh-my-codex` / `omx`，并检查本机是否已安装 superpowers skills。
...
项目级 skill 只放真正属于本项目的能力。`oh-my-codex` / `omx` 由 npm 包和 `harness/scripts/install-ai-skills.sh` 管理，不在 `.agents/skills/` 中重复定义。

...
4. **编码实现使用 `superpowers:subagent-driven-development` 或 `superpowers:executing-plans`。**
...
1. 使用 `superpowers:systematic-debugging` 复现并定位。
2. 写清根因、影响范围和修复方案。
3. **修复实现使用 `superpowers:executing-plans` 或直接处理（视复杂程度而定）。**
...
1. 读取 `harness/project.profile.md` 和相关代码。
...
1. 从 harness/entrypoints/ 投影根目录 AI 入口文件。
2. 从 harness/adapters/ 投影工具适配目录。
3. 创建 .ai-runtime-artifacts/ 及其子目录。
4. 如需安装或检查 AI runtime，请先说明会修改哪些本机环境，然后由你执行 harness/scripts/install-ai-skills.sh。
5. 读取 harness/init/project-profiler.prompt.md。
6. 扫描当前项目，生成或更新 harness/project.profile.md、harness/context-map.md、harness/project.verification.md。
7. 由你运行 harness/scripts/harness-check.sh。
8. 汇总推断项、待确认项和验证结果。
```

...
你正在把 Agent Harness 脚手架接入当前项目。`harness/` 是源头，根目录入口和工具目录都是投影。

## 投影入口文件

从 `harness/entrypoints/` 投影到项目根目录：

- `harness/entrypoints/AGENTS.md` -> `AGENTS.md`
- `harness/entrypoints/CLAUDE.md` -> `CLAUDE.md`
- `harness/entrypoints/GEMINI.md` -> `GEMINI.md`

如果目标文件已存在，先读取现有内容，只合并 Harness 入口，不删除项目已有约束。

## 投影工具适配

从 `harness/adapters/` 投影到项目根目录：
...
完成入口和工具适配投影后，读取 `harness/init/project-profiler.prompt.md`，生成或更新：

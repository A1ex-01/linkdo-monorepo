...
[[ -f "$web_env_file" ]] || { echo "Missing $web_env_file" >&2; exit 1; }
...
docker compose --env-file "$backend_env_file" --env-file "$admin_env_file" --env-file "$web_env_file" --env-file "$image_env_file" -f "$deploy_dir/compose.yaml" exec -T \
